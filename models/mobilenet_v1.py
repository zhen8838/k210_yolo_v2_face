            ...
            for i, conv_def in enumerate(conv_defs):
                ...
                if isinstance(conv_def, Conv):
                    ...
                    net = slim.conv2d(net, depth(conv_def.depth), [1, 1],
                                      stride=1,
                                      scope=end_point)
                    if end_point == 'Conv2d_11_pointwise':
...
def mobilenet_v1_arg_scope(
    ...
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], activation_fn=tf.nn.relu6, normalizer_fn=normalizer_fn):
        with slim.arg_scope([slim.batch_norm], **batch_norm_params):
            with slim.arg_scope([slim.conv2d]):
                with slim.arg_scope([slim.separable_conv2d]) as sc:
                    return sc
